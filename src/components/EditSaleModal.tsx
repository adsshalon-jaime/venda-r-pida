...
export function EditSaleModal({ open, onOpenChange, sale, products, customers, onUpdate }: EditSaleModalProps) {
  ...
  const filteredProducts = sale?.isRental 
    ? products.filter(product => product.isRental)
    : products;
  ...
  useEffect(() => {
    if (sale && open) {
      ...
      setWidth(sale.width || 0);
      setLength(sale.length || 0);
      setCustomValueValue(sale.totalValue);
      setSelectedCustomerId(sale.customerId || '');
      setSaleDate(new Date(sale.saleDate).toISOString().split('T')[0]);
    }
  }, [sale, open, setCustomValueValue]);
  ...
  const handleSubmit = () => {
    ...
    onOpenChange(false);
    toast.success(sale.isRental ? 'Locação atualizada com sucesso!' : 'Venda atualizada com sucesso!');
  };
      ...
      <DialogContent className="sm:max-w-[500px] animate-scale-in">
        <DialogHeader>
          <DialogTitle className="text-xl font-semibold">
            {sale?.isRental ? 'Editar Locação' : 'Editar Venda'}
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="space-y-2">
            <Label>Data da {sale?.isRental ? 'Locação' : 'Venda'}</Label>
            <Input
              type="date"
              value={saleDate}
          ...
          {selectedProduct && (
            <div className="space-y-4 animate-fade-in">
              ...
              {/* Value Field */}
              <div className="space-y-2">
                <Label>Valor da {sale?.isRental ? 'Locação' : 'Venda'}</Label>
          ...
          <div className="flex gap-3 pt-2">
            ...
            >
              {sale?.isRental ? 'Atualizar Locação' : 'Atualizar Venda'}
            </Button>
          </div>
        </div>
      </DialogContent>
