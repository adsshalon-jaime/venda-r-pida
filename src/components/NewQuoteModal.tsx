...
interface NewQuoteModalProps {
  ...
  customers: Customer[];
  onQuoteComplete: (quote: Omit<Quote, 'id' | 'createdAt' | 'updatedAt' | 'quoteNumber'>) => Promise<void>;
}

export function NewQuoteModal({ open, onOpenChange, customers, onQuoteComplete }: NewQuoteModalProps) {
  ...
  useEffect(() => {
    if (open) {
      ...
      setItems([]);
      setDeliveryAddress('');
      setDeliveryDeadline('5 dias úteis');
      setShippingCost(0);
      setValidityDays(15);
  ...
  const handleSubmit = async () => {
    ...
    await onQuoteComplete(quoteData);
    onOpenChange(false);
  };
            ...
            {!selectedCustomer && (
              <div className="bg-amber-50 border border-amber-200 rounded-lg p-3">
                <div className="flex items-center gap-2 text-xs text-amber-800">
                  ...
                  <span className="font-medium">Atenção: Selecione um cliente cadastrado</span>
                </div>
                <p className="text-xs text-amber-700 mt-1">
                  Para criar um orçamento, é obrigatório selecionar um cliente.
                </p>
              </div>
            )}
          </div>
