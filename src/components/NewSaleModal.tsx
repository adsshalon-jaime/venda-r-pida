...
export function NewSaleModal({
  ...
  const handleSubmit = () => {
    ...
    if (saleType === 'rental') {
      if (!selectedCustomer) {
        toast.error('Para locação, é necessário selecionar um cliente');
        return;
      }
      if (!deliveryDate) {
        toast.error('Informe a data de entrega/montagem');
        return;
      }
      if (!removalDate) {
        toast.error('Informe a data de remoção');
        return;
      }
      if (!installationAddress) {
        toast.error('Informe o endereço de montagem');
        return;
      }
      ...
      if (removalDateObj <= deliveryDateObj) {
        toast.error('Data de remoção deve ser posterior à data de entrega');
        return;
      }
    }
    ...
    // Criar informações de locação (se for locação)
    const rentalInfo = saleType === 'rental' ? {
      deliveryDate: new Date(deliveryDate + 'T00:00:00'),
      deliveryTime: deliveryTime || undefined,
      removalDate: new Date(removalDate + 'T00:00:00'),
      installationAddress,
      serviceOrderNumber: `OS-${Date.now()}`, // Gerar número de OS único
      isRemoved: false,
    } : undefined;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto animate-scale-in">
        ...
        <div className="space-y-4 py-4">
          ...
          {/* Rental Information */}
          {saleType === 'rental' && (
            <div className="space-y-3 bg-amber-50 border-2 border-amber-200 rounded-lg p-4 animate-fade-in">
              <div className="flex items-center gap-2 mb-2">
                <Key className="h-5 w-5 text-amber-600" />
                <Label className="text-base font-semibold text-amber-800">Informações da Locação</Label>
              </div>

              {/* Endereço de Montagem */}
              <div className="space-y-1">
                <Label className="text-xs">Endereço de Montagem *</Label>
                <Input
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="space-y-1">
                  <Label className="text-xs">Data de Entrega/Montagem *</Label>
                  <Input
                    type="date"
                    value={deliveryDate}
              ...
              {/* Data de Remoção */}
              <div className="space-y-1">
                <Label className="text-xs">Data de Remoção/Desmontagem *</Label>
                <Input
                  type="date"
                  value={removalDate}
              ...
              {/* Resumo da Locação */}
              {deliveryDate && removalDate && (
                <div className="bg-white border border-amber-300 rounded-lg p-3 space-y-2">
                  <div className="flex items-center gap-2 text-xs text-amber-800">
                    <Calendar className="h-3 w-3" />
                    <span className="font-semibold">Resumo da Locação</span>
                  </div>
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    ...
                    <div>
                      <span className="text-gray-600">Remoção:</span>
                      <div className="font-medium text-red-700">
                        {new Date(removalDate + 'T00:00:00').toLocaleDateString('pt-BR')}
                      </div>
                    </div>
                    <div className="col-span-2">
                      <span className="text-gray-600">Período:</span>
                      <div className="font-medium text-amber-700">
                        {Math.ceil((new Date(removalDate + 'T00:00:00').getTime() - new Date(deliveryDate + 'T00:00:00').getTime()) / (1000 * 60 * 60 * 24))} dias
                      </div>
                    </div>
                  </div>
                  <div className="bg-blue-50 border border-blue-200 rounded p-2 mt-2">
                    <div className="text-xs text-blue-800 font-semibold">
                      📋 Ordem de Serviço será gerada automaticamente
                    </div>
                  </div>
                </div>
              )}
              ...
              {!selectedCustomer && (
                <div className="bg-red-50 border border-red-200 rounded-lg p-2">
                  ...
                  <p className="text-xs text-red-700 mt-1">
                    Para locação, é obrigatório selecionar um cliente acima.
                  </p>
                </div>
              )}
            </div>
