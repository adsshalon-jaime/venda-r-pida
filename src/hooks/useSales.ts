...
export function useSales() {
  ...
  const fetchSales = useCallback(async () => {
    try {
      ...
      const mappedSales: Sale[] = (data || []).map((s: any) => ({
        ...
        rentalInfo: s.rental_delivery_date ? {
          deliveryDate: new Date(s.rental_delivery_date + 'T00:00:00'),
          deliveryTime: s.rental_delivery_time || undefined,
          removalDate: new Date(s.rental_removal_date + 'T00:00:00'),
          installationAddress: s.rental_installation_address || undefined,
          serviceOrderNumber: s.rental_service_order_number || undefined,
          isRemoved: s.rental_is_removed || false,
        } : undefined,
      }));
    ...
    try {
      const { data, error } = await supabase
        ...
        .insert({
          ...
          rental_delivery_date: saleData.rentalInfo?.deliveryDate?.toISOString().split('T')[0] || null,
          rental_delivery_time: saleData.rentalInfo?.deliveryTime || null,
          rental_removal_date: saleData.rentalInfo?.removalDate?.toISOString().split('T')[0] || null,
          rental_installation_address: saleData.rentalInfo?.installationAddress || null,
          rental_service_order_number: saleData.rentalInfo?.serviceOrderNumber || null,
          rental_is_removed: saleData.rentalInfo?.isRemoved || false,
        } as any)
        .select()
        .single();
      ...
      const newSale: Sale = {
        ...
        rentalInfo: (data as any).rental_delivery_date ? {
          deliveryDate: new Date((data as any).rental_delivery_date + 'T00:00:00'),
          deliveryTime: (data as any).rental_delivery_time || undefined,
          removalDate: new Date((data as any).rental_removal_date + 'T00:00:00'),
          installationAddress: (data as any).rental_installation_address || undefined,
          serviceOrderNumber: (data as any).rental_service_order_number || undefined,
          isRemoved: (data as any).rental_is_removed || false,
        } : undefined,
      };
  ...
  const markSaleAsPaid = async (saleId: string) => {
    try {
      // Alterar o método de pagamento de 'fiado' para 'dinheiro' para marcar como pago
      const { data, error } = await supabase
        .from('sales')
        .update({
  ...
  const getOverdueSales = useCallback(() => {
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    
    return sales.filter(s => {
      if (s.paymentInfo?.method !== 'fiado' || !s.paymentInfo?.dueDate) return false;
      const dueDate = new Date(s.paymentInfo.dueDate);
      dueDate.setHours(0, 0, 0, 0);
      return dueDate < today;
    });
  ...
  const getPendingRemovals = useCallback(() => {
    return sales.filter(s => {
      if (!s.isRental || !s.rentalInfo || s.rentalInfo.isRemoved) return false;
      return true;
    });
  }, [sales]);

  const markAsRemoved = async (saleId: string) => {
    try {
      const { error } = await supabase
        .from('sales')
        .update({
          rental_is_removed: true,
        } as any)
        .eq('id', saleId);
      ...
      toast.success('Locação marcada como removida!');
    } catch (error: any) {
      console.error('Error marking as removed:', error);
      toast.error('Erro ao marcar como removida');
      throw error;
    }
  };
